...
module.exports = {
    ...
    create(discount){
    return new Promise((resolve,reject) => {
        db.query("SELECT * FROM discounts WHERE code = $1", [discount.code],(err,res)=>{
            if(res.rows[0]){
              reject("Email already exists")
            }else if(err){
              reject(err)
            }else {
            db.query(`INSERT INTO discounts (discount,code) VALUES($1,$2)`,[discount.discount,discount.code], (err,res) => {
                if(err != null){
                    reject(err);
                }else {
                    resolve(res.rows);
        ...
        update(discount){
            return new Promise((resolve,reject) => {
                db.query(`UPDATE discounts SET discount = $1 WHERE code = $2`,[discount.discount,discount.code], (err,res) => {
                    if(err != null){
                        reject(err);
                    }else {
                        resolve(res.rows);
